...
class Captcha
{
     ...
     */
    public function check(string $captcha_id, string $code): bool
    {
        // 取出验证码正确结果
        $key = Cache::get('captcha_' . $captcha_id);
        // 若未找到有效的验证码结果
        if (!$key) return false;
    ...
    protected function generate(): array
    {
        ...
        // 生成唯一的验证码ID
        do {
            $captcha_id = self::random(32);
        } while (Cache::has('captcha_' . $captcha_id));

        // 保存到缓存
        Cache::set('captcha_' . $captcha_id, $key, $this->expire);
     ...
     * @return string 随机字符串
     */
    private static function random(int $length, int $numeric = 0): string
    {
        //生成若干位随机字符串
